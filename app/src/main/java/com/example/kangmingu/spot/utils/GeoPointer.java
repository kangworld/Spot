...
public class GeoPointer extends AsyncTask<String, Void, GeoPointer.Point[]> {
    private final static String NAVER_CLIENT_ID = "19gvJB99utu_FF9K3DpX";
    private final static String NAVER_CLIENT_SECRET = "B9YItadazC";
    ...
    protected Point[] doInBackground(String... params) {
        ...
        for (int i = 0; i < params.length; i++) {
            ...
            Point point;
            point = getPointFromGeoCoder(addr);

            // 구글의 지오코더로부터 주소를 갖고오지 못했으면 네이버 api를 이용해서 가져온다.
            if (!point.havePoint)
                point = getPointFromNaver(addr);

            points[i] = point;
        }
        Log.d("길이", Integer.toString(points.length));
        return points;
    }
    /**
    ...
    private Point getPointFromGeoCoder(String addr) {
        ...
        point.addr = listAddress.get(0).getAddressLine(0);
        return point;
    }
}
